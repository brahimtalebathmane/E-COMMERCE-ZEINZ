...
| `created_at` timestamptz | default `now()`. **Immutable row-insert timestamp** — audit only, and relied on by Meta CAPI dispatch idempotency (`order_meta_dispatches`), WhatsApp dispatch (`order_whatsapp_dispatches`) and `order_communication_logs`. Never edited by any admin action. |
| `ordered_at` timestamptz (063/064) | not null, default `now()`, indexed `orders_ordered_at_idx desc`. **The business date of the sale** — defaults to now() at insert, editable by admins afterward (backdating a manual sale, correcting a storefront order's date). Every report, filter, grouping and sort (`/admin/orders` day-grouping, `/admin/analytics` daily/lifetime profit, the dashboard's "orders today") reads this column, never `created_at`. Edited via `updateOrderDateAction`, logged to `order_communication_logs` as `order_date_changed`. |
| `unit_price` numeric NULL (063) | Selling price per unit at order creation (`total_price / quantity`). Snapshot/audit only — profit calculations use `total_price` directly. |
| `unit_cost_price` numeric NULL (063) | `products.cost_price` **at the moment this order was created**. Profit analytics prefer this over the product's current `cost_price` (falls back to it only when null, e.g. pre-migration orders). |
| `affiliate_commission_type_at_order` text NULL (063) | `products.affiliate_commission_type` snapshot at order creation, same `fixed`/`set_price` check constraint. Falls back to the product's current value when null. |
| `affiliate_fixed_commission_at_order` numeric NULL (063) | `products.affiliate_fixed_commission` snapshot at order creation. Falls back to the product's current value when null. |
| `affiliate_sell_price_at_order` numeric NULL (063) | `products.affiliate_sell_price` snapshot at order creation. Falls back to the product's current value when null. |

Indexes: `orders_product_id_idx`, `orders_created_at_idx`, `orders_ordered_at_idx` (desc), `orders_completion_token_key` (unique).

**`created_at` vs `ordered_at`:** these are deliberately two different columns. `created_at` is the immutable moment the row was written to the database — Meta CAPI dispatch idempotency, WhatsApp dispatch idempotency, and `order_communication_logs` all key off it, and nothing in the admin panel ever changes it. `ordered_at` is the business date of the sale — it defaults to `created_at`'s value (via `now()`) for every normal order, but an admin can move it, e.g. entering a manual sale for a call that happened yesterday, or correcting a storefront order's date. Every piece of reporting (day-grouping, daily/lifetime profit, "orders today") reads `ordered_at`.

**Profit is not retroactive (migration 063):** before 063, every profit figure other than `orders.total_price` was read LIVE from `products` at report time (`cost_price`, `affiliate_fixed_commission`, `affiliate_sell_price`, `affiliate_commission_type`), so editing a product's price/cost/commission terms silently rewrote months of already-reported profit. Migration 063 snapshots those four figures onto each order at creation (`unit_cost_price`, `affiliate_commission_type_at_order`, `affiliate_fixed_commission_at_order`, `affiliate_sell_price_at_order`); `src/lib/analytics/profit.ts` and `daily-profit.ts` now read the order's own snapshot first and only fall back to the product's current value when the snapshot is null (orders created before the migration). Editing a product's economics in `/admin/products` now only ever affects orders created after the change. `profit_calculation_start_date` (migration 033) remains as an optional analytics cutoff — unrelated to this guarantee.
