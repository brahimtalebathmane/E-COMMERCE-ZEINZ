/** Static Arabic copy for the admin dashboard (RTL). */
export const adminAr = {
  nav: {
    title: "الإدارة",
    products: "المنتجات",
    orders: "الطلبات",
    paymentMethods: "طرق الدفع",
    storefront: "المتجر",
  },
  login: {
    title: "تسجيل دخول الإدارة",
    subtitle: "استخدم حساب Supabase المخصص لهذا المشروع.",
    email: "البريد الإلكتروني",
    password: "كلمة المرور",
    signingIn: "جاري تسجيل الدخول…",
    signIn: "دخول",
    loading: "جاري التحميل…",
  },
  products: {
    title: "المنتجات",
    newProduct: "منتج جديد",
    name: "الاسم",
    slug: "المسار",
    price: "السعر",
    edit: "تعديل",
    view: "عرض",
    noProducts: "لا توجد منتجات بعد.",
  },
  newProduct: {
    title: "منتج جديد",
    slugHint:
      "يُولَّد المسار (slug) تلقائياً من الاسم ولا يمكن تغييره لاحقاً.",
  },
  editProduct: {
    title: "تعديل المنتج",
  },
  orders: {
    title: "الطلبات",
    subtitle:
      "تُعرض الإيصالات والمرفقات عبر روابط موقّعة (تخزين خاص).",
    loadError: "تعذر تحميل الطلبات:",
    customer: "العميل",
    phone: "الهاتف",
    address: "العنوان",
    product: "المنتج",
    payment: "الدفع",
    ref: "المرجع",
    total: "الإجمالي",
    formData: "بيانات النموذج",
    actions: "إجراءات",
    noOrders: "لا توجد طلبات بعد.",
  },
  orderStatus: {
    pending: "قيد الانتظار",
    confirmed: "مؤكد",
    shipped: "تم الشحن",
    cancelled: "ملغى",
  },
  orderActions: {
    sendFormLink: "إرسال رابط النموذج (واتساب)",
    formDone: "اكتمل النموذج",
    previewReceipt: "معاينة الإيصال",
    signUrlFailed: "تعذر إنشاء رابط موقّع",
  },
  paymentMethods: {
    title: "طرق الدفع",
    subtitleBefore: "تظهر في نافذة الدفع. يمكنك أيضاً تعيين",
    subtitleAfter: "في البيئة كاحتياط عندما تكون قاعدة البيانات فارغة.",
    existing: "الموجودة",
    noRows: "لا توجد صفوف في قاعدة البيانات بعد.",
    loadError: "تعذر تحميل طرق الدفع:",
    addMethod: "إضافة طريقة",
    label: "التسمية",
    numberField: "الرقم",
    accountNumber: "الحساب / الرقم المعروض للمشتري",
    sortOrder: "ترتيب العرض",
    saving: "جاري الحفظ…",
    add: "إضافة",
    placeholderLabel: "تحويل بنكي",
    active: "نشط",
    save: "حفظ",
    delete: "حذف",
    deleteConfirm: "حذف طريقة الدفع هذه؟",
    logoUrl: "رابط شعار الدفع (صورة)",
    logoUrlHint:
      "PNG أو JPG أو SVG — اتركه فارغاً إن لم يكن هناك شعار. يجب أن يبدأ الرابط بـ https://",
    logoUrlInvalid:
      "رابط الصورة غير صالح. استخدم https:// مع مسار ينتهي بـ .png أو .jpg أو .jpeg أو .svg",
    logoPreview: "معاينة الشعار",
  },
  productForm: {
    slugFixed: "المسار (ثابت):",
    name: "الاسم",
    description: "الوصف",
    price: "السعر",
    discount: "سعر التخفيض (اختياري)",
    discountPlaceholder: "اتركه فارغاً لاستخدام السعر العادي",
    mediaType: "نوع الوسائط",
    mediaImage: "صورة",
    mediaVideo: "فيديو (رابط Mux / Cloudflare Stream)",
    mediaUrl: "رابط الوسائط",
    features: "المميزات (سطر لكل ميزة)",
    gallery: "روابط صور المعرض (سطر لكل رابط)",
    testimonialsJson: "التقييمات (مصفوفة JSON)",
    faqJson: "الأسئلة الشائعة (مصفوفة JSON)",
    exampleLabel: "مثال",
    metaPixel: "معرّف Meta Pixel",
    metaPlaceholder: "أرقام فقط",
    formTitle: "عنوان النموذج بعد الدفع",
    legacySlugs: "مسارات قديمة (سطر لكل مسار، إعادة توجيه اختيارية)",
    saving: "جاري الحفظ…",
    create: "إنشاء منتج",
    saveChanges: "حفظ التغييرات",
    delete: "حذف",
    deleteConfirm:
      "حذف هذا المنتج؟ تبقى الطلبات محفوظة، لكن الصفحة ستعيد 404.",
    failedSave: "فشل الحفظ",
    failedDelete: "فشل الحذف",
  },
  formBuilder: {
    title: "حقول النموذج بعد الدفع",
    newFieldDefault: "حقل جديد",
    addField: "إضافة حقل",
    fieldLabel: "التسمية",
    type: "النوع",
    required: "إلزامي",
    up: "أعلى",
    down: "أسفل",
    remove: "إزالة",
    emptyHint:
      "لا توجد حقول بعد. يمكن للعملاء التأكيد بعد الدفع دون حقول (غير موصى به).",
  },
  fieldTypes: {
    text: "نص",
    textarea: "نص طويل",
    file: "ملف",
    email: "بريد",
    link: "رابط",
  },
} as const;
