/** Static Arabic copy for the admin dashboard (RTL). */
export const adminAr = {
  nav: {
    title: "الإدارة",
    home: "الرئيسية",
    products: "المنتجات",
    orders: "الطلبات",
    analytics: "الأرباح",
    meta: "مراقبة Meta",
    marketing: "الرسائل التسويقية",
    assistant: "المساعد الذكي",
    storefront: "المتجر",
    staff: "الموظفون",
    countries: "الدول",
  },
  shell: {
    brandTagline: "لوحة التحكم",
    sectionMain: "العمليات",
    sectionSettings: "الإعدادات",
    menu: "القائمة",
    closeMenu: "إغلاق القائمة",
    openMenu: "فتح القائمة",
    logout: "تسجيل الخروج",
    loggingOut: "جاري الخروج…",
    viewStore: "زيارة المتجر",
    more: "المزيد",
    moreNav: "المزيد من الخيارات",
    countryScope: "الدولة المعروضة",
  },
  common: {
    loading: "جاري التحميل…",
    errorTitle: "حدث خطأ غير متوقع",
    errorBody:
      "تعذر عرض هذا القسم. يمكنك إعادة المحاولة، وإذا استمرت المشكلة أعد تحميل الصفحة.",
    retry: "إعادة المحاولة",
    invalidDate: "تاريخ غير متوفر",
  },
  dashboard: {
    title: "نظرة عامة",
    subtitle: "ملخص حي لأداء المتجر اليوم.",
    greeting: "مرحباً بك في لوحة التحكم",
    kpiRevenue: "إجمالي الإيرادات (مملوك، MRU)",
    kpiNetProfit: "صافي الربح (مملوك، MRU)",
    kpiMissingCostWarning: "⚠ {count} منتج بلا سعر تكلفة — الربح أعلاه مبالغ فيه",
    kpiOrders: "إجمالي الطلبات",
    kpiOrdersToday: "طلبات اليوم",
    kpiPending: "بانتظار المعالجة",
    kpiActiveProducts: "منتجات نشطة",
    pipelineTitle: "خط إنتاج المنتجات",
    pipelineResearch: "بحث وتقصي",
    pipelineReady: "قيد الاختبار",
    pipelineWinner: "فائز / نشط",
    pipelineFailed: "فاشل",
    recentTitle: "أحدث الطلبات",
    recentEmpty: "لا توجد طلبات بعد.",
    recentViewAll: "عرض كل الطلبات",
    quickActions: "إجراءات سريعة",
    quickNewProduct: "إضافة منتج",
    quickOrders: "إدارة الطلبات",
    quickAnalytics: "تحليلات الأرباح",
    loadError: "تعذر تحميل بيانات اللوحة:",
  },
  login: {
    title: "تسجيل دخول الإدارة",
    subtitle: "استخدم حساب Supabase المخصص لهذا المشروع.",
    email: "البريد الإلكتروني",
    password: "كلمة المرور",
    signingIn: "جاري تسجيل الدخول…",
    signIn: "دخول",
    loading: "جاري التحميل…",
    forbidden: "ليس لديك صلاحية الوصول إلى لوحة الإدارة.",
    suspended: "تم تعليق حسابك. تواصل مع المدير.",
  },
  staff: {
    title: "إدارة الموظفين",
    subtitle: "إنشاء حسابات العمال وتحديد صلاحياتهم بدقة.",
    addEmployee: "إضافة موظف",
    newEmployee: "موظف جديد",
    editEmployee: "تعديل الموظف",
    edit: "تعديل",
    create: "إنشاء الحساب",
    save: "حفظ التغييرات",
    cancel: "إلغاء",
    closeForm: "إغلاق النموذج",
    displayName: "الاسم المعروض",
    newPasswordOptional: "كلمة مرور جديدة (اختياري)",
    accountActive: "الحساب نشط",
    permissionsTitle: "الصلاحيات",
    active: "نشط",
    suspended: "معلّق",
    suspend: "تعليق",
    reactivate: "إعادة التفعيل",
    empty: "لا يوجد موظفون بعد. أنشئ أول حساب للفريق.",
    unnamed: "موظف بدون اسم",
    noPermissions: "بدون صلاحيات",
    colEmployee: "الموظف",
    colStatus: "الحالة",
    colActions: "إجراءات",
    permissionLegendTitle: "دليل الصلاحيات",
    permissionGranted: "مفعّل",
    permissionDenied: "غير مفعّل",
  },
  countries: {
    title: "إدارة الدول",
    subtitle: "الدول التي يبيع فيها المتجر — كل دولة لها عملتها وبكسل Meta الخاص بها.",
    addCountry: "إضافة دولة",
    newCountry: "دولة جديدة",
    editCountry: "تعديل الدولة",
    create: "إنشاء",
    save: "حفظ التغييرات",
    cancel: "إلغاء",
    nameAr: "الاسم بالعربية",
    nameFr: "الاسم بالفرنسية",
    isoCode: "رمز الدولة (ISO، حرفان)",
    currency: "العملة (ISO، 3 أحرف)",
    metaPixelPublic: "معرّف Meta Pixel (المتصفح)",
    metaPixelServer: "معرّف Meta Pixel (الخادم / CAPI)",
    metaPixelHint: "اتركه فارغاً لاستخدام بكسل الموقع الموحّد من الإعدادات العامة.",
    isActive: "نشطة",
    inactive: "موقوفة",
    colCountry: "الدولة",
    colCurrency: "العملة",
    colStatus: "الحالة",
    colActions: "إجراءات",
    edit: "تعديل",
    delete: "حذف",
    deleteConfirm: "حذف هذه الدولة نهائياً؟",
    empty: "لا توجد دول بعد.",
  },
  products: {
    title: "المنتجات",
    newProduct: "منتج جديد",
    name: "الاسم",
    slug: "المسار",
    price: "السعر (MRU)",
    edit: "تعديل",
    view: "عرض",
    noProducts: "لا توجد منتجات بعد.",
  },
  pipeline: {
    image: "الصورة",
    sourcingType: "نوع التوريد",
    sourcingLink: "رابط التوريد",
    codMargin: "هامش COD المتوقع",
    marginLabel: "الهامش",
    marginUnset: "أدخل سعر التكلفة",
    moveToTest: "نقل إلى مرحلة الاختبار",
    setupLanding: "إعداد صفحة الهبوط",
    priceColumn: "السعر",
    markWinner: "فائز / نشط",
    markFailed: "فاشل",
    stage: "المرحلة",
    statusReady: "جاهز للاختبار",
    statusTesting: "قيد الاختبار",
    emptyTab: "لا توجد منتجات في هذه المرحلة.",
    statusUpdateFailed: "تعذر تحديث حالة المنتج",
    delete: "حذف",
    deleteConfirm:
      "حذف هذا المنتج؟ تبقى الطلبات المرتبطة محفوظة، لكن صفحة الهبوط ستعيد 404.",
    deleteFailed: "فشل حذف المنتج",
    deleting: "جاري الحذف…",
  },
  editResearch: {
    title: "تعديل بيانات البحث",
  },
  newProduct: {
    title: "منتج جديد (بحث وتقصي)",
    slugHint:
      "يُولَّد المسار تلقائياً من الاسم عند الإنشاء. يمكنك تخصيصه لاحقاً من إعداد صفحة الهبوط أو تعديل المنتج.",
    researchHint:
      "أدخل بيانات التوريد والتسعير فقط. بعد الحفظ، أكمل صفحة الهبوط من تبويب «البحث والتقصي».",
  },
  researchForm: {
    title: "بيانات المنتج — مرحلة البحث",
    hint: "الحقول التالية فقط مطلوبة الآن. باقي أقسام صفحة الهبوط تُضاف لاحقاً عبر «إعداد صفحة الهبوط».",
    productName: "اسم المنتج",
    mainImage: "الصورة الرئيسية (رابط أو رفع)",
    price: "سعر البيع (MRU)",
    costPrice: "سعر التكلفة (MRU)",
    sourcingType: "نوع التوريد",
    sourcingLink: "رابط التوريد الداخلي",
    uploading: "جارٍ رفع الصورة…",
    create: "إضافة إلى البحث والتقصي",
    saving: "جاري الحفظ…",
    cancel: "إلغاء",
    failedSave: "فشل إنشاء المنتج",
    saveChanges: "حفظ التغييرات",
    delete: "حذف المنتج",
    deleteConfirm:
      "حذف هذا المنتج؟ لا يمكن التراجع عن هذا الإجراء.",
    failedDelete: "فشل الحذف",
  },
  landingSetup: {
    title: "إعداد صفحة الهبوط",
    subtitle:
      "جميع الحقول اختيارية — يمكنك حفظ التقدم في أي وقت وإكمال الصفحة لاحقاً. الحفظ لا يغيّر مرحلة المنتج في خط الإنتاج؛ استخدم «إنهاء البحث» عند الجاهزية للاختبار.",
    backToPipeline: "العودة إلى المنتجات",
    cost: "التكلفة",
    displayCurrency: "العملة المعروضة",
    submit: "حفظ إعدادات صفحة الهبوط",
    completeResearch: "إنهاء البحث والنقل إلى جاهز للاختبار",
    saving: "جاري الحفظ…",
  },
  editProduct: {
    title: "تعديل المنتج",
  },
  orders: {
    title: "الطلبات",
    subtitle:
      "القائمة تعرض أهم الحقول؛ اضغط على أي طلب لعرض التفاصيل الكاملة.",
    loadError: "تعذر تحميل الطلبات:",
    customer: "العميل",
    phone: "الهاتف",
    address: "العنوان",
    product: "المنتج",
    payment: "الدفع",
    ref: "المرجع",
    total: "الإجمالي (MRU)",
    formData: "بيانات النموذج",
    formDataEmpty: "لا يوجد",
    actions: "إجراءات",
    noOrders: "لا توجد طلبات بعد.",
    status: "الحالة",
    tapForDetails: "اضغط للتفاصيل ←",
    openDetailHint: "← عرض",
    closeDetail: "إغلاق",
    orderDetailTitle: "تفاصيل الطلب",
    sectionMeta: "البيانات العامة",
    sectionProduct: "المنتج",
    sectionTotal: "الإجمالي",
    sectionActions: "إجراءات الطلب",
    delete: "حذف الطلب",
    deleteConfirm: "هل تريد حذف هذا الطلب نهائياً؟ لا يمكن التراجع.",
    deleteBulkConfirm:
      "هل تريد حذف {count} طلباً نهائياً؟ لا يمكن التراجع.",
    deleting: "جاري الحذف…",
    deletingBulk: "جاري حذف الطلبات…",
    deleteFailed: "فشل حذف الطلب",
    deleteSelected: "حذف المحدّد",
    selectAll: "تحديد الكل",
    deselectAll: "إلغاء التحديد",
    selectedCountLabel: "{count} محدّد",
    orderDate: "تاريخ الطلب",
    viewStoreProduct: "عرض في المتجر",
    productPrice: "السعر الأساسي",
    productDiscount: "سعر التخفيض",
    mediaVideo: "رابط الفيديو",
    mediaImage: "رابط الصورة",
    saveChanges: "حفظ التغييرات",
    savingChanges: "جاري الحفظ…",
    saveFailed: "فشل حفظ التغييرات",
    deliveryCost: "تكلفة التوصيل",
    deliveryCostPlaceholder: "0",
    deliveryCostSaved: "تم حفظ تكلفة التوصيل",
    deliveryCostSaveFailed: "تعذر حفظ تكلفة التوصيل",
    deliveryCostInvalid: "تكلفة التوصيل يجب أن تكون رقماً صحيحاً وأكبر من أو يساوي صفر",
    orderDateEdit: "تاريخ الطلب (قابل للتعديل)",
    orderDateSaved: "تم تحديث تاريخ الطلب",
    orderDateSaveFailed: "تعذر تحديث تاريخ الطلب",
    orderDateInvalid: "التاريخ غير صالح",
    metaPurchaseCapiOk: "تم إرسال حدث الشراء (Purchase) إلى Meta بنجاح.",
    metaPurchaseCapiFailed:
      "تعذر إرسال حدث الشراء إلى Meta. تحقق من META_CAPI_ACCESS_TOKEN في Netlify وسجلات الدالة.",
    metaPurchaseCapiMissingMeta:
      "لم يُرسل حدث الشراء: الطلب يفتقد بيانات التتبع (معرّف الحدث أو البكسل).",
    metaCancelCapiOk: "تم إرسال حدث الإلغاء (CancelledLead) إلى Meta بنجاح.",
    metaCancelCapiFailed:
      "تعذر إرسال حدث الإلغاء إلى Meta. تحقق من META_CAPI_ACCESS_TOKEN في Netlify وسجلات الدالة.",
    metaCancelCapiMissingMeta:
      "لم يُرسل حدث الإلغاء: الطلب يفتقد بيانات التتبع (معرّف الحدث أو البكسل).",
    metaLeadCapiOk: "تم إرسال حدث Lead إلى Meta بنجاح.",
    metaLeadCapiFailed:
      "تعذر إرسال حدث Lead إلى Meta. تحقق من META_CAPI_ACCESS_TOKEN في Netlify وسجلات الدالة.",
    metaLeadCapiMissingMeta:
      "لم يُرسل حدث Lead: الطلب يفتقد بيانات التتبع (معرّف الحدث أو البكسل).",
    metaTrackingTitle: "تتبع Meta",
    metaLeadSent: "Lead: تم الإرسال",
    metaLeadNotSent: "Lead: لم يُرسل",
    metaPurchaseSent: "Purchase: تم الإرسال",
    metaPurchaseNotSent: "Purchase: لم يُرسل",
    metaCancelSent: "CancelledLead: تم الإرسال",
    metaCancelNotSent: "CancelledLead: لم يُرسل",
    metaRetryLead: "إعادة إرسال Lead",
    metaRetryPurchase: "إعادة إرسال Purchase",
    metaRetryCancel: "إعادة إرسال CancelledLead",
    metaRetrying: "جاري الإرسال…",
    allProducts: "كل المنتجات",
    productUnknown: "منتج غير معروف",
    today: "اليوم",
    yesterday: "أمس",
    ordersCountLabel: "طلب",
    metricPending: "قيد الانتظار",
    metricConfirmed: "مؤكد",
    metricShipped: "تم الشحن",
    metricCancelled: "ملغى",
    metricNeedsAttention: "متابعة بشرية",
    metricInternalReturn: "مرتجع داخلي",
    segmentSummaryTitle: "ملخص الطلبات",
    newOrderBadge: "جديد",
    noOrdersHint: "ستظهر الطلبات الجديدة هنا فور وصولها — لا حاجة لتحديث الصفحة.",
    readOnlyStatus: "الحالة",

    // Confirmation dialog (replaces window.confirm)
    confirmTitle: "تأكيد الإجراء",
    confirm: "تأكيد",
    cancel: "إلغاء",

    // Order note (visible on the row/card, click to edit)
    notePlaceholder: "أضف ملاحظة…",
    noteAdd: "+ إضافة ملاحظة",
    noteSaved: "تم حفظ الملاحظة",
    noteSaveFailed: "تعذر حفظ الملاحظة",

    // Call / WhatsApp quick actions
    callTitle: "اتصال",
    whatsappTitle: "واتساب",
    phoneUnavailable: "لا يوجد رقم هاتف صالح",

    // Search
    searchPlaceholder: "ابحث بالاسم أو رقم الهاتف…",
    searchNoResults: "لا توجد نتائج مطابقة للبحث.",

    // Selection mode
    selectionModeEnter: "تحديد",
    selectionModeExit: "إلغاء التحديد",

    // Bulk status change
    bulkStatusLabel: "تغيير الحالة",
    bulkStatusPlaceholder: "اختر الحالة الجديدة",
    bulkStatusApply: "تطبيق",
    bulkStatusApplying: "جاري التطبيق…",
    bulkStatusConfirm: "هل تريد تغيير حالة {count} طلباً إلى «{status}»؟",
    bulkStatusResult: "تم تحديث {okCount} طلباً بنجاح، وتعذر تحديث {failCount}.",
    bulkStatusAllOk: "تم تحديث {okCount} طلباً بنجاح.",
    bulkStatusFailed: "تعذر تغيير الحالة",

    // Quantity (editable per order line)
    quantity: "الكمية",
    quantitySaved: "تم تحديث الكمية",
    quantitySaveFailed: "تعذر تحديث الكمية",
    quantityInvalid: "الكمية يجب أن تكون رقماً صحيحاً 1 أو أكثر",

    // Manual (offline) sales
    manualSaleBadge: "بيع يدوي",
    addManualSale: "+ بيع يدوي",

    // Deleted-orders entry point (Part B)
    deletedLink: "المحذوفة ({count})",
  },
  deletedOrders: {
    title: "الطلبات المحذوفة",
    backToOrders: "← رجوع إلى الطلبات",
    forbidden: "ليس لديك صلاحية الوصول إلى هذه الصفحة.",
    loadError: "تعذر تحميل الطلبات المحذوفة:",
    empty: "لا توجد طلبات محذوفة في {country}",
    emptyAllCountries: "لا توجد طلبات محذوفة في أي دولة",
    outsideScope: "توجد {count} طلبات محذوفة خارج نطاق الدولة المحددة.",
    outsideScopeLink: "عرض كل الدول",
    showingAllCountries: "تعرض هذه الصفحة الآن الطلبات المحذوفة من كل الدول.",
    backToScoped: "العودة لنطاق الدولة المحددة",
    productDeleted: "منتج محذوف",
    countLabel: "{count} طلب محذوف",
    colDeletedAt: "تاريخ الحذف",
    colOrderedAt: "تاريخ الطلب",
    colStatusAtDeletion: "الحالة عند الحذف",
    selectRow: "تحديد هذا الطلب",
    restore: "استرجاع",
    restoreSelected: "استرجاع المحدّد ({count})",
    restoreConfirm: "هل تريد استرجاع {count} طلباً؟ سيعود للظهور في قائمة الطلبات.",
    restoreSuccess: "تم استرجاع الطلب بنجاح",
    restoreFailed: "تعذر استرجاع الطلب",
    prevPage: "السابق",
    nextPage: "التالي",
    pageInfo: "صفحة {page} من {totalPages}",
  },
  manualSale: {
    title: "بيع واتساب جديد",
    subtitle:
      "سجّل بيعاً تم عبر محادثة واتساب — ابحث عن المحادثة أو أدخل رقماً يدوياً، وسيُربط الإعلان تلقائياً إن وُجدت محادثة له.",
    conversation: "المحادثة",
    selectConversation: "اختر محادثة…",
    loadingConversations: "جاري تحميل المحادثات…",
    noConversations: "لا توجد محادثات واردة بعد. تظهر هنا فور وصول أول رسالة.",
    conversationsFailed: "تعذر تحميل المحادثات",
    conversationRequired: "اختر المحادثة التي تم البيع خلالها.",
    modeConversation: "من المحادثات",
    modeManual: "إدخال رقم يدوياً",
    searchPlaceholder: "ابحث بالاسم أو الرقم…",
    searching: "جاري البحث…",
    noSearchResults: "لا توجد محادثة مطابقة",
    useManualInstead: "استخدم هذا الرقم يدوياً",
    manualPhone: "رقم واتساب",
    manualPhoneInvalid: "رقم الهاتف غير صالح (8 أرقام تبدأ بـ 2 أو 3 أو 4)",
    manualPhoneRequired: "أدخل رقم واتساب صالحاً.",
    manualContactFound: "هذا الرقم له محادثة واردة — سيُربط الإعلان تلقائياً.",
    manualContactMissing:
      "لا توجد محادثة بهذا الرقم — سيُسجَّل البيع بدون نسبته إلى إعلان.",
    adSource: "الإعلان",
    adSourceNone: "بدون إعلان",
    adWindowExpired:
      "مضى أكثر من ٧ أيام على نقرة الإعلان — سيُرسل الشراء لكن Meta لن تنسبه إلى الحملة.",
    customerName: "اسم العميل",
    customerNamePlaceholder: "اسم العميل",
    phone: "رقم الهاتف / واتساب",
    orderDate: "تاريخ الطلب",
    product: "المنتج",
    selectProduct: "اختر منتجاً",
    quantity: "الكمية",
    addLine: "+ إضافة منتج آخر",
    removeLine: "إزالة",
    total: "الإجمالي",
    initialStatus: "حالة الطلب عند الإنشاء",
    statusPending: "قيد الانتظار",
    statusConfirmed: "مؤكد (سيُرسل حدث Purchase إلى Meta فوراً)",
    submit: "تسجيل البيع",
    submitting: "جاري التسجيل…",
    success: "تم تسجيل البيع بنجاح",
    cancel: "إلغاء",
    close: "إغلاق",
    loadProductsFailed: "تعذر تحميل قائمة المنتجات",
    noProducts: "لا توجد منتجات متاحة",
    metaPurchaseFailedNote: "تنبيه: تعذر إرسال حدث Purchase إلى Meta لبعض العناصر.",
  },
  orderStatus: {
    pending: "قيد الانتظار",
    confirmed: "مؤكد",
    shipped: "تم الشحن",
    cancelled: "ملغى",
    requires_human_intervention: "يحتاج متابعة بشرية",
    internal_return: "مرتجع داخلي",
  },
  analytics: {
    title: "تحليلات الأرباح",
    subtitle:
      "صافي الربح = الإيرادات (الطلبات المشحونة فقط) − تكلفة البضاعة − تكلفة التوصيل − الإنفاق الإعلاني (مباشر من Meta). المرتجعات الداخلية تُستثنى تلقائياً من الإيرادات والتكلفة.",

    sectionOverviewTitle: "نظرة عامة",
    sectionFinancialTitle: "الملخص المالي",

    kpiGrossRevenue: "إجمالي الإيرادات",
    kpiGrossRevenueHint: "مجموع أسعار بيع الطلبات المشحونة فقط",
    kpiCogs: "تكلفة البضاعة (COGS)",
    kpiCogsHint: "تكلفة شراء الوحدات المباعة",
    kpiDeliveryCost: "تكلفة التوصيل",
    kpiDeliveryCostHint: "مجموع تكلفة التوصيل للطلبات المشحونة",
    kpiAdSpend: "الإنفاق الإعلاني",
    kpiAdSpendHint: "مباشر من Meta، مجموع كل الحملات المرتبطة بكل منتج",
    kpiNetProfit: "صافي الربح",
    kpiNetProfitHint: "الإيرادات بعد خصم التكلفة والتوصيل والإعلان",
    unitsSold: "وحدات مباعة",
    tableTitle: "الأرباح حسب المنتج",
    tableHint: "الإنفاق الإعلاني يُحدَّث تلقائياً من Meta. اضغط على اسم المنتج لعرض التفاصيل اليومية.",
    colProduct: "المنتج",
    colUnits: "وحدات",
    colRevenue: "الإيرادات",
    colCogs: "التكلفة",
    colDeliveryCost: "التوصيل",
    colAdSpend: "الإنفاق الإعلاني",
    colStartDate: "تاريخ بدء الحساب",
    colNetProfit: "صافي الربح",
    colNetMargin: "هامش الربح",
    sortBy: "ترتيب حسب",
    colReturns: "مرتجعات",
    save: "حفظ",
    saving: "جاري الحفظ…",
    startDateHint:
      "احسب الأرباح من هذا التاريخ فصاعداً فقط. اتركه فارغاً لحساب كل الفترة.",
    startDateClear: "مسح",
    startDateSaved: "تم تحديث تاريخ بدء الحساب",
    startDateCleared: "تمت العودة لحساب كل الفترة",
    startDateSaveFailed: "تعذر حفظ تاريخ بدء الحساب",
    noData: "لا توجد بيانات أرباح بعد. ستظهر هنا عند شحن أول طلب.",
    costMissing: "لم تُحدَّد التكلفة",
    returnsNote: "مرتجع داخلي (مستبعد من الأرباح، بدون أي حدث Meta)",

    summaryToday: "ربح اليوم",
    summary7d: "متوسط 7 أيام",
    summary30d: "متوسط 30 يوماً",
    summaryBestProduct: "الأفضل أداءً",
    summaryWorstProduct: "الأضعف أداءً",
    summaryNoProduct: "لا يوجد بعد",

    granularityDaily: "يومي",
    granularityWeekly: "أسبوعي",
    combinedChartTitle: "صافي الربح الإجمالي عبر الزمن",

    trendUp: "اتجاه صاعد",
    trendDown: "اتجاه هابط",
    trendFlat: "مستقر",
    trendInsufficientData: "بيانات غير كافية",
    lossStreakWarning: "خسارة لـ {count} أيام متتالية",

    adSpendFreshAsOf: "آخر تحديث: {time}",
    adSpendRefreshFailed: "تعذر تحديث الإنفاق الإعلاني من Meta — تُعرض آخر بيانات محفوظة.",

    manageCampaigns: "إدارة الحملات",
    campaignsTitle: "حملات Meta المرتبطة",
    campaignIdPlaceholder: "معرّف الحملة (Campaign ID)",
    linkCampaign: "ربط",
    linking: "جاري الربط…",
    unlinkCampaign: "إلغاء الربط",
    noCampaignsLinked: "لا توجد حملات مرتبطة بهذا المنتج بعد.",
    campaignLinked: "تم ربط الحملة وجلب بيانات الإنفاق",
    campaignLinkFailed: "تعذر ربط الحملة",
    campaignUnlinked: "تم إلغاء ربط الحملة",
    campaignUnlinkFailed: "تعذر إلغاء ربط الحملة",

    viewProductDetail: "عرض التفاصيل",
    backToAnalytics: "← رجوع إلى تحليلات الأرباح",
    productDetailDayByDay: "الأرباح اليومية",
    productAvgDailyProfit: "متوسط الربح اليومي لهذا المنتج",
    overallAvgDailyProfit: "متوسط الربح اليومي لكل المنتجات",
    productNotFound: "المنتج غير موجود",

    // Period filter
    periodAll: "كل الفترة",
    periodThisMonth: "هذا الشهر",
    periodLastMonth: "الشهر الماضي",
    periodPickMonth: "اختر شهراً محدداً",
    periodSyncing: "جارٍ جلب الإنفاق الإعلاني من Meta لهذا الشهر…",
    periodSoFar: "حتى اليوم",
    periodMonthDaysSoFar: "{days} يوماً حتى الآن",
    periodMonthDaysTotal: "{days} يوماً",
    periodAdSpendIncomplete:
      "قد يكون الإنفاق الإعلاني لهذا الشهر غير مكتمل (قد يسبق ربط الحملة، أو لا تتوفر بيانات من Meta لهذه الفترة).",

    // Data-quality strip (Part C1)
    dataQualityTruncated:
      "بيانات مقتطعة: تجاوز عدد الطلبات أو صفوف الإنفاق الإعلاني حد الجلب — الأرقام أدناه قد تكون غير مكتملة.",
    dataQualityMissingCost:
      "تكلفة ناقصة: {count} منتج ساهم بإيراد {revenue} بدون سعر تكلفة محدد — الربح أدناه مبالغ فيه بمقدار غير معروف.",
    dataQualityUnparseableDates:
      "طلبات بتاريخ غير صالح: {count} طلب لا يمكن تحديد تاريخه، لذا يظهر في «كل الفترة» فقط وليس في أي شهر محدد.",

    // Month-mode overview tiles
    monthSummaryProfit: "ربح الشهر",
    monthSummaryAvgDaily: "متوسط الربح اليومي",
    monthSummaryWinningDays: "أيام رابحة",
    monthSummaryLosingDays: "أيام خاسرة",

    // Profitability metrics block (Part 2)
    metricsTitle: "مؤشرات الربحية",
    metricNetMargin: "هامش الربح الصافي",
    metricRoas: "ROAS (العائد على الإنفاق الإعلاني)",
    metricProfitPerAdSpend: "الربح لكل أوقية إنفاق",
    metricCpo: "تكلفة اكتساب الطلب (CPO)",
    metricAov: "متوسط قيمة الطلب (AOV)",
    metricAvgOrderProfit: "متوسط ربح الطلب",
    metricOrdersCount: "عدد الطلبات",
    chipMargin: "الهامش",
    chipRoas: "ROAS",

    // Previous-month comparison (Part 3)
    comparedToPreviousMonth: "مقارنة بـ {month}",
    noPreviousMonthData: "لا توجد بيانات كافية للشهر السابق للمقارنة",
    showPreviousMonthOnChart: "إظهار الشهر السابق على الرسم البياني",
  },
  aiAgent: {
    title: "مدير وكيل الذكاء الاصطناعي",
    subtitle:
      "عرّف سلوك الوكيل لهذا المنتج بالعربية أو الحسانية. يُستخدم عند رد العملاء على واتساب بعد الطلب.",
    backToProduct: "العودة لتعديل المنتج",
    openManager: "وكيل واتساب (AI)",
    intro:
      "الوكيل يراجع رسائل العميل ويطبّق القواعد أدناه. عند الشك يُحوّل الطلب لفريقكم تلقائياً.",
    activeLabel: "تفعيل الوكيل لهذا المنتج",
    rulesLabel: "تعليمات السلوك (system instruction)",
    rulesHint:
      "صف متى تُؤكَّد الطلبات، كيف تتعامل مع الصور، واللهجة المطلوبة. اترك الحقل فارغاً لتعطيل الرد الآلي.",
    rulesPlaceholder:
      "مثال: إذا أرسل العميل صورة تأكيد أو وافق على السعر، قم بتحديث حالة الطلب إلى مؤكد…",
    exampleTitle: "مثال جاهز",
    exampleBody:
      "إذا أرسل العميل صورة تأكيد أو وافق على السعر، قم بتحديث حالة الطلب إلى «مؤكد» وأخبره أن التوصيل سيتم عبر خدمة التوصيل المعتمدة. إذا طلب تغيير العنوان أو كان غاضباً، اطلب تدخل بشري.",
    save: "حفظ التعليمات",
    saving: "جاري الحفظ…",
    saveSuccess: "تم حفظ إعدادات الوكيل",
    saveFailed: "تعذر الحفظ",
    runtimeTitle: "تشغيل الردود على واتساب (ليس Netlify فقط)",
    runtimeBody:
      "حفظ القواعد هنا يعمل على Netlify. لكن الرد التلقائي يحتاج: (1) Railway متصل بواتساب، (2) WHATSAPP_WEBHOOK_SECRET على Railway وNetlify، (3) WHATSAPP_WEBHOOK_FORWARD_URL على Railway = رابط Netlify أدناه، (4) OPENAI_API_KEY و OPENAI_ASSISTANT_ID على Netlify.",
    runtimeWebhookUrl: "رابط توجيه الرسائل على Railway:",
  },
  orderActions: {
    delete: "حذف",
  },
  productForm: {
    langArabic: "عربي",
    langFrench: "Français",
    defaultLanguage: "اللغة الافتراضية للصفحة",
    defaultLanguageHint:
      "تُفتح صفحة المنتج دائماً بهذه اللغة افتراضياً (بدون كشف تلقائي من الجهاز). يمكن للعميل تغيير اللغة يدوياً.",
    defaultLanguageArabic: "عربي (AR)",
    defaultLanguageFrench: "Français (FR)",
    slugFixed: "المسار (ثابت):",
    slugLabel: "مسار رابط صفحة الهبوط",
    slugHint:
      "يمكنك تعديل المسار في أي وقت. عند تغييره يُحفظ المسار السابق تلقائياً لإعادة التوجيه (301) حتى تبقى روابط الحملات تعمل.",
    slugPreviewLabel: "معاينة الرابط:",
    slugLegacyNote: "مسارات قديمة تُعيد التوجيه تلقائياً:",
    introBody:
      "كل الحقول في هذه الصفحة تؤثر مباشرة على صفحة المنتج للزبائن: الهوية، الرأس، النصوص، الوسائط، الشريط السفلي، ثم التتبع والواتساب.",
    sectionBrand: "الشعار ولون الصفحة",
    sectionBrandHint:
      "يُستخدم اللون في أزرار العناوين والتمييز؛ الشعار يظهر أعلى صفحة الهبوط. صيغة اللون #RRGGBB.",
    sectionHeader: "شريط العروض أعلى الصفحة",
    sectionHeaderHint:
      "سطر واحد بعرض الشاشة فوق الهيدر (خلفية خضراء داكنة). اتركه فارغاً لإخفاء الشريط. «نداء الرأس» سطر اختياري بلون التمييز داخل الهيدر.",
    topBannerOffer: "نص العرض أو الترويج (سطر واحد)",
    topBannerOfferPlaceholder: "مثال: خصم 20٪ على الطلب اليوم — التوصيل مجاني",
    name: "الاسم",
    description: "الوصف",
    price: "السعر (MRU)",
    pricePlaceholder: "مثال: 1000",
    sectionPipeline: "البحث والاختبار",
    sectionPipelineHint:
      "حالة الاختبار تحدد ظهور المنتج في المتجر (الفائز فقط). سعر التكلفة يُستخدم لحساب هامش COD في لوحة الإدارة.",
    testStatus: "مرحلة الاختبار",
    testStatusUnderResearch: "بحث وتقصي",
    testStatusReady: "جاهز للاختبار",
    testStatusTesting: "قيد الاختبار",
    testStatusWinner: "فائز / نشط",
    testStatusFailed: "فاشل",
    sourcingType: "نوع التوريد",
    sourcingTypeUnset: "— غير محدد —",
    sourcingLocal: "محلي",
    sourcingImport: "استيراد",
    sourcingLink: "رابط التوريد الداخلي",
    sourcingLinkPlaceholder: "https://…",
    costPrice: "سعر التكلفة (MRU)",
    costPricePlaceholder: "لحساب الهامش",
    affiliateCostPricePlaceholder: "التكلفة التي أبلغنا بها شريك COD Partner",
    affiliateCostPriceHint:
      "سعر التكلفة الذي أبلغنا به COD Partner لهذا المنتج (نفس حقل «سعر التكلفة» أعلاه).",
    economicsSnapshotHint:
      "يُطبَّق هذا التغيير على الطلبات الجديدة فقط، ولا يؤثر على الأرباح المحسوبة للطلبات السابقة.",
    sectionFulfillment: "طريقة التنفيذ",
    sectionFulfillmentHint:
      "منتج مملوك: نؤكد ونشحن ونسلّم بأنفسنا (الدفع عند الاستلام، MRU). منتج تابع (Affiliate): COD Partner يملك المنتج ويؤكد ويشحن ويسلّم؛ نحن فقط نسوّق له وترسل كل الطلبات إلى Google Sheet خاص به.",
    fulfillmentType: "نوع التنفيذ",
    fulfillmentOwned: "مملوك (دفع عند الاستلام)",
    fulfillmentAffiliate: "تابع (COD Partner)",
    affiliateCommissionType: "نوع العمولة",
    affiliateCommissionTypeUnset: "— اختر —",
    affiliateCommissionFixed: "عمولة ثابتة",
    affiliateCommissionSetPrice: "نحدد نحن السعر",
    affiliateSku: "رمز المنتج (SKU) لدى COD Partner",
    affiliateMarketCountry: "السوق (من شاشة الدول)",
    affiliateMarketCountryUnset: "— اختر السوق —",
    affiliateCountry: "الدولة المستهدفة (نص حر لملف COD Partner)",
    affiliateCountryUnset: "— اختر الدولة —",
    affiliateCurrency: "العملة",
    affiliateCurrencyPlaceholder: "مثال: KWD",
    affiliateSheetUrl: "رابط Google Sheet",
    affiliateFixedCommission: "قيمة العمولة الثابتة",
    affiliateSellPrice: "سعر البيع للعميل",
    discount: "سعر التخفيض (MRU، اختياري)",
    discountPlaceholder: "اتركه فارغاً لاستخدام السعر العادي",
    displayCurrency: "اسم العملة كما يظهر للزبون",
    displayCurrencyPlaceholder: "مثال: أوقية أو UM",
    displayCurrencyHint:
      "يظهر بجانب السعر في صفحة الهبوط وصفحة تأكيد الطلب فقط، ولا يغيّر عملة الطلب الفعلية. اتركه فارغاً لاستخدام عملة الدولة تلقائياً.",
    mediaType: "نوع الوسائط",
    mediaImage: "صورة",
    mediaVideo: "فيديو (رابط Mux / Cloudflare Stream)",
    mediaUrl: "رابط الوسائط",
    mediaUploadButton: "رفع صورة",
    mediaUploadInProgress: "جارٍ رفع الصورة…",
    mediaUploadOrLink: "أو الصق رابطاً مباشراً (فيديو Mux / Cloudflare Stream فقط)",
    mediaPreview: "معاينة",
    uploadFailedGeneric: "فشل رفع الصورة.",
    uploadFailedBanner: "فشل رفع صورة البانر.",
    uploadFailedLogo: "فشل رفع الشعار.",
    secondaryMediaType: "الوسيط الثاني: النوع",
    secondaryMediaUrl: "الوسيط الثاني: الرابط",
    tertiaryMediaType: "الوسيط الثالث: النوع",
    tertiaryMediaUrl: "الوسيط الثالث: الرابط",
    features: "المميزات",
    featuresHint: "أضف كل ميزة في حقل منفصل. يمكنك تغيير الترتيب أو الحذف.",
    featuresHintBilingual:
      "لكل ميزة حقل عربي (إلزامي لحفظ السطر) وحقل فرنسي اختياري. إذا تركت الفرنسية فارغة يظهر العربي في واجهة الفرنسية.",
    addFeature: "إضافة ميزة",
    featurePlaceholder: "نص الميزة",
    featurePlaceholderFr: "Texte (français)",
    gallery: "معرض الصور",
    galleryHint: "أضف رابطاً لكل صورة. تظهر معاينة عند نجاح التحميل.",
    addGalleryImage: "إضافة صورة",
    galleryUrlPlaceholder: "https://",
    galleryPreview: "معاينة",
    testimonialsSection: "آراء العملاء",
    testimonialsHint: "اسم صاحب الرأي والنص. الحقل «الصفة» اختياري.",
    testimonialsHintBilingual:
      "أدخل النص العربي أولاً (مطلوب لحفظ الرأي). يمكنك إضافة ترجمة فرنسية لكل حقل؛ إن تركتها فارغة يُعرض العربي.",
    addTestimonial: "إضافة رأي",
    testimonialName: "اسم الشخص",
    testimonialQuote: "النص",
    testimonialRole: "الصفة (اختياري)",
    faqSection: "الأسئلة الشائعة",
    faqHint: "أضف سؤالاً وجواباً لكل عنصر.",
    faqHintBilingual:
      "السؤال والجواب العربيان مطلوبان لحفظ العنصر. أضف الفرنسية اختيارياً؛ إن لم تُدخل يُعرض العربي.",
    addFaq: "إضافة سؤال",
    faqQuestion: "السؤال",
    faqAnswer: "الجواب",
    specsSection: "المواصفات التقنية",
    specsHintBilingual:
      "التسمية والقيمة العربيتان مطلوبتان لحفظ العنصر. أضف الفرنسية اختيارياً؛ إن لم تُدخل تُعرض العربية.",
    addSpec: "إضافة مواصفة",
    specLabel: "التسمية",
    specValue: "القيمة",
    sectionStickyColors: "ألوان الشريط السفلي الثابت",
    sectionStickyColorsHint:
      "اترك أي حقل فارغاً للافتراضي في القالب. صيغة #RRGGBB.",
    sectionIntegrations: "التتبع والرسائل",
    sectionIntegrationsHint:
      "البكسل يُحمّل في صفحة المنتج. قالب واتساب يُستخدم على الخادم بعد الطلب وليس كنص في الواجهة.",
    statsSectionTitle: "عنوان قسم الإحصائيات (اختياري)",
    testimonialsBadgeLabel: "السطر تحت عنوان التقييمات (اختياري)",
    footerNoteLabel: "سطر أسفل الصفحة / حقوق النشر (اختياري)",
    metaPixel: "معرّف Meta Pixel",
    metaPlaceholder: "أرقام فقط",
    whatsappTemplate: "قالب رسالة واتساب (اختياري)",
    whatsappTemplateHint:
      "سيُرسل هذا النص تلقائياً للعميل عبر واتساب بعد نجاح الطلب. اتركه فارغاً لتعطيل الإرسال لهذا المنتج.",
    legacySlugs: "مسارات قديمة لإعادة التوجيه",
    legacySlugsHint:
      "إذا غيّر المنتج مساره، أضف المسارات القديمة هنا حتى تبقى الروابط تعمل.",
    addLegacySlug: "إضافة مسار",
    legacySlugPlaceholder: "مسار-قديم",
    removeItem: "حذف",
    moveUp: "أعلى",
    moveDown: "أسفل",
    saving: "جاري الحفظ…",
    create: "إنشاء منتج",
    saveChanges: "حفظ التغييرات",
    delete: "حذف",
    deleteConfirm:
      "حذف هذا المنتج؟ تبقى الطلبات محفوظة، لكن الصفحة ستعيد 404.",
    failedSave: "فشل الحفظ",
    failedDelete: "فشل الحذف",
    saved: "تم حفظ التغييرات بنجاح",
    created: "تم إنشاء المنتج بنجاح",
    deleted: "تم حذف المنتج بنجاح",
    archived: "تم أرشفة المنتج (الطلبات المرتبطة محفوظة)",
  },
  assistant: {
    title: "مساعد الإدارة الذكي",
    subtitle: "نفّذ أوامر المتجر بالمحادثة — المنتجات والطلبات.",
    openButton: "مساعد الإدارة",
    close: "إغلاق",
    newChat: "محادثة جديدة",
    placeholder: "اكتب أمراً… مثال: أكّد الطلب، غيّر سعر منتج، أنشئ منتجاً…",
    send: "إرسال",
    sending: "جاري التنفيذ…",
    emptyTitle: "مرحباً 👋",
    emptyBody:
      "أنا مساعد الإدارة. يمكنني إنشاء المنتجات وتعديلها، عرض ملخص الطلبات وتحديث حالاتها (تأكيد، شحن، إلغاء) مع تشغيل أحداث Meta تلقائياً. اكتب طلبك بالعربية أو الفرنسية.",
    youLabel: "أنت",
    assistantLabel: "المساعد",
    errorGeneric: "تعذر تنفيذ الطلب. حاول مرة أخرى.",
  },
  meta: {
    title: "مراقبة Meta",
    subtitle: "متابعة أحداث Pixel و CAPI والأعطال.",
    loadError: "تعذر تحميل سجل Meta.",
    overviewTitle: "نظرة عامة",
    overviewSubtitle: "ملخص آخر 24 ساعة وحالة الطلبات العالقة.",
    failures24h: "إخفاقات (24 س)",
    skips24h: "تخطٍ متوقع (24 س)",
    successes24h: "نجاح (24 س)",
    stuckNow: "طلبات عالقة الآن",
    checkHealth: "فحص إعدادات Meta",
    checkingHealth: "جاري الفحص…",
    healthOk: "إعدادات Meta تبدو سليمة.",
    healthFail: "مشكلة في إعدادات Meta.",
    healthFetchError: "تعذر الاتصال بفحص الصحة.",
    testEventTitle: "اختبار الأحداث",
    testEventSend: "إرسال حدثي اختبار",
    testEventSending: "جاري الإرسال…",
    testEventHelp:
      "يرسل حدثي شراء تجريبيين إلى Meta بشكلين مختلفين ليظهرا في Events Manager ← Test Events. لا يُنشئ أي طلب ولا يُسجَّل في السجل أدناه. يعمل فقط عند ضبط META_CAPI_TEST_EVENT_CODE.",
    testEventPixel: "البكسل المُرسَل إليه",
    testEventCode: "كود الاختبار المُرسَل",
    testEventShapeWebsite: "شكل المتجر (بمتصفح و IP)",
    testEventShapeChat: "شكل بيع واتساب (بلا متصفح ولا IP)",
    testEventRead:
      "افتح Test Events وقارن: القيمة 1.11 هي شكل المتجر، و2.22 شكل بيع واتساب. إن ظهرت 1.11 وحدها فالبكسل والكود سليمان، والأداة لا تعرض أحداث المحادثات. وإن لم يظهر شيء فالبكسل أعلاه ليس البكسل المفتوح لديك.",
    datasetTitle: "معرّف dataset لواتساب",
    datasetFetch: "احصل على معرّف dataset لواتساب",
    datasetFetching: "جاري الطلب…",
    datasetCopyHint: "انسخ هذا وضعه في META_WHATSAPP_DATASET_ID ثم أعد النشر.",
    datasetAlready: "مضبوط بالفعل ✓",
    datasetHelp:
      "أحداث المحادثات لا تُرسل إلى بكسل الموقع بل إلى dataset خاص بحساب واتساب للأعمال. اضغط الزر مرة واحدة للحصول على معرّفه.",
    lastSuccessTitle: "آخر نجاح لكل نوع حدث",
    logTitle: "سجل الأحداث",
    filterAllTypes: "كل الأنواع",
    filterAllStates: "كل الحالات",
    stateFailed: "فشل",
    stateSkipped: "تخطٍ",
    stateSuccess: "نجاح",
    searchPlaceholder: "بحث: معرف طلب / منتج / event_id",
    applyFilters: "تطبيق",
    colTime: "الوقت",
    colType: "النوع",
    colState: "الحالة",
    colReason: "السبب",
    colOrder: "الطلب",
    colEventId: "event_id",
    noRows: "لا توجد سجلات مطابقة.",
    prevPage: "السابق",
    nextPage: "التالي",
    pageInfo: (page: number, totalPages: number, total: number) =>
      `صفحة ${page} من ${totalPages} (${total} سجل)`,
    eventTypes: {
      lead: "Lead",
      purchase: "شراء",
      cancelled_lead: "إلغاء Lead",
      initiate_checkout: "بدء الشراء",
      view_content: "عرض المحتوى",
      config_health: "صحة الإعدادات",
      emq_check: "جودة المطابقة",
      pixel_load_failure: "فشل تحميل البكسل",
      dataset_resend: "إعادة إرسال dataset واتساب",
    },
    ctwaTitle: "أداء إعلانات واتساب",
    ctwaSubtitle:
      "كل إعلان «انقر للمراسلة» خلال آخر {days} يوماً: كم محادثة فتح، وكم منها صار بيعاً.",
    ctwaLoadError: "تعذر تحميل تقرير أداء الإعلانات.",
    ctwaConversations: "محادثات من الإعلانات",
    ctwaOrders: "طلبات",
    ctwaConfirmed: "غير ملغاة",
    ctwaRevenue: "الإيراد",
    ctwaRate: "نسبة التحويل",
    ctwaTruncated: "بلغت البيانات الحد الأقصى للصفوف — الأرقام المعروضة جزئية.",
    ctwaEmptyTitle: "لا توجد نقرات إعلانات بعد",
    ctwaEmptyBody:
      "يبدأ التقاط معرّفات الإعلانات من أول رسالة تصل من إعلان «انقر للمراسلة» بعد تفعيل الالتقاط.",
    ctwaColAd: "معرّف الإعلان",
    ctwaColConversations: "محادثات",
    ctwaColOrders: "طلبات",
    ctwaColConfirmed: "غير ملغاة",
    ctwaColCancelled: "ملغاة",
    ctwaColRate: "التحويل",
    ctwaColRevenue: "الإيراد",
    gapTitle: "فجوة dataset واتساب",
    gapSubtitle:
      "مبيعات واتساب من إعلان وصلت إلى البكسل ولم تصل إلى dataset واتساب — المصدر الوحيد لهدف «زيادة المشتريات عبر الرسائل». الصفر هو الوضع الطبيعي.",
    gapLoadError: "تعذر تحميل حالة dataset واتساب.",
    gapEligible: "في الفجوة الآن (قابلة للإعادة)",
    gapHealthy: "لا توجد فجوة ✓",
    gapUnhealthy: "كل طلب هنا تحويل لا يتعلم منه محسّن الإعلانات. إن بقي الرقم فوق الصفر أكثر من يوم فخط dataset يتدهور.",
    gapExpired: "خارج نافذة الإرسال — لا يمكن استعادتها (آخر {days} يوماً)",
    gapNotConfigured: "dataset واتساب غير مضبوط على هذا الخادم (META_WHATSAPP_DATASET_ID / META_WHATSAPP_BUSINESS_ACCOUNT_ID).",
    gapLastError: "آخر خطأ من dataset",
    gapLastErrorNone: "لا يوجد خطأ قائم.",
    gapDryRun: "تشغيل تجريبي",
    gapDryRunning: "جاري الفحص…",
    gapResend: "إعادة الإرسال إلى dataset",
    gapResending: "جاري الإرسال…",
    gapResendNeedsDryRun: "شغّل الفحص التجريبي أولاً.",
    gapDryRunResult: "قابلة للإعادة: {eligible} · خارج النافذة: {expired}",
    gapOldest: "الأقدم",
    gapNewest: "الأحدث",
    gapRunResult: "مقبولة: {accepted} · مرفوضة: {rejected} · متخطاة: {skipped} · متبقية: {remaining}",
    gapFirstSubcode: "أول subcode",
    gapStopped: "توقف التشغيل:",
    gapStopReasons: {
      business_messaging_rejection: "رفض Meta بخطأ 2804xxx — الطلبات التالية ستصطدم بالخطأ نفسه.",
      time_budget: "انتهى الوقت المتاح لهذا التشغيل — أعد الضغط لمتابعة الباقي.",
      not_configured: "dataset واتساب غير مضبوط.",
    },
    gapLastRun: "آخر تشغيل",
    gapLastRunNone: "لم يُشغَّل بعد.",
    gapPixelOnly: "لا يُرسل هذا الإجراء إلى البكسل أبداً — هذه الطلبات سُجّلت عليه مسبقاً.",
    coverageTitle: "سقف إشارة واتساب",
    coverageSubtitle:
      "بيع واتساب بلا معرّف نقرة (ctwa_clid) لا يصل إلى dataset أبداً. نسبة التغطية هي أقصى ما سيراه هدف الرسائل.",
    coverageLoadError: "تعذر تحميل تغطية الإشارة.",
    coverageWindow: "آخر {days} يوماً",
    coveragePurchases: "مشتريات واتساب",
    coverageAttributable: "تحمل ctwa_clid",
    coveragePercent: "نسبة التغطية",
    coverageReached: "وصلت إلى dataset",
    coverageTruncated: "بلغت البيانات الحد الأقصى للصفوف — الأرقام المعروضة جزئية.",
  },
  marketing: {
    title: "الرسائل التسويقية",
    subtitle: "إرسال حملات واتساب تسويقية للعملاء السابقين.",
    loadError: "تعذر تحميل الحملات.",

    senderStatusLabel: "حالة إرسال واتساب",
    senderConnected: "متصل",
    senderDisconnected: "غير متصل — لا يمكن الإرسال حالياً",
    senderCheckFailed: "تعذر التحقق من حالة الإرسال",

    newCampaign: "حملة جديدة",
    messageText: "نص الرسالة",
    messageTextPlaceholder: "اكتب نص الرسالة هنا…",
    image: "صورة (اختياري)",
    uploadingImage: "جاري رفع الصورة…",
    imageUploadError: "تعذر رفع الصورة.",
    removeImage: "إزالة الصورة",

    audienceType: "الجمهور المستهدف",
    audienceAllConfirmed: "كل العملاء الذين أكدوا طلباً",
    audienceByProduct: "عملاء منتج معين",
    audienceManual: "اختيار يدوي",
    selectProduct: "اختر المنتج",
    searchCustomers: "ابحث بالاسم أو رقم الهاتف",
    searching: "جاري البحث…",
    addCustomer: "إضافة",
    added: "تمت الإضافة",
    noSearchResults: "لا توجد نتائج.",
    manualSelectedTitle: "المستلمون المختارون",
    removeRecipient: "إزالة",

    recipientCount: "عدد المستلمين: {count}",
    loadingRecipients: "جاري حساب عدد المستلمين…",
    noRecipients: "لا يوجد مستلمون مطابقون لهذا الاختيار.",

    excludeShippedTitle: "استبعاد العملاء الذين تم شحن منتج معين لهم (اختياري)",
    excludeShippedHint: "لتفادي تعارض الأسعار — مثال: إرسال خصم لعميل اشترى المنتج بالسعر الكامل بالفعل.",

    reviewTitle: "المراجعة والإرسال",
    reviewMessagePreview: "معاينة الرسالة",
    reviewAudience: "الجمهور",
    reviewRecipientCount: "عدد المستلمين",
    createCampaign: "حفظ كمسودة",
    creatingCampaign: "جاري الحفظ…",
    createError: "تعذر إنشاء الحملة.",
    send: "بدء الإرسال",
    sending: "جاري البدء…",
    sendConfirmTitle: "تأكيد الإرسال",
    sendConfirm: "سيتم إرسال هذه الرسالة إلى {count} عميل، رسالة واحدة كل 20-45 ثانية تقريباً. هل تريد المتابعة؟",
    sendError: "تعذر بدء الإرسال.",
    sendSuccess: "بدأ إرسال الحملة.",
    senderOfflineWarning: "خدمة واتساب غير متصلة حالياً — لا يمكن بدء حملة جديدة.",

    resume: "استئناف",
    resuming: "جاري الاستئناف…",
    resumeError: "تعذر استئناف الحملة.",
    resumeConfirm: "توقفت هذه الحملة تلقائياً بعد عدة إخفاقات متتالية. هل تريد استئنافها؟",

    historyTitle: "سجل الحملات",
    historyEmpty: "لا توجد حملات بعد.",
    colDate: "التاريخ",
    colAudience: "الجمهور",
    colStatus: "الحالة",
    colRecipients: "المستلمون",
    colSent: "تم الإرسال",
    colFailed: "فشل",

    statusDraft: "مسودة",
    statusSending: "قيد الإرسال",
    statusCompleted: "مكتملة",
    statusFailed: "متوقفة",

    recipientsListTitle: "قائمة المستلمين",
    recipientStatusPending: "قيد الانتظار",
    recipientStatusSent: "تم الإرسال",
    recipientStatusFailed: "فشل",
    close: "إغلاق",
  },
} as const;
