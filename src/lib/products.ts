...
export function mapProductRow(row: Record<string, unknown>): ProductAdminRow {
  ...
  return {
    ...
    specs_title_ar: (row.specs_title_ar as string) ?? "",
    specs_title_fr: (row.specs_title_fr as string) ?? "",
    ...
    specs_ar: (row.specs_ar as ProductRow["specs_ar"]) ?? [],
    specs_fr: (row.specs_fr as ProductRow["specs_fr"]) ?? [],
