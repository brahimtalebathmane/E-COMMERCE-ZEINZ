...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: order, error } = await supabase
      .from("orders")
      .select(
        "id, status, customer_name, phone, total_price, currency, meta_event_id, meta_event_source_url, meta_pixel_id, meta_purchase_sent",
      )
      .eq("id", data.order_id)
      .maybeSingle();
    ...
    if (order.status !== "confirmed") {
      return NextResponse.json({ sent: false, reason: "status_not_confirmed" }, { status: 200 });
    }
    ...
    console.error("[POST /api/meta/purchase]", err);
