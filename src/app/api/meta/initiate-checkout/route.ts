...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: order, error } = await supabase
      .from("orders")
      .select(
        "id, customer_name, phone, total_price, currency, meta_event_id, meta_event_source_url, meta_pixel_id, meta_initiate_checkout_sent, meta_fbp, meta_fbc",
      )
      .eq("id", data.order_id)
      .maybeSingle();
    ...
    if (order.meta_initiate_checkout_sent) {
    ...
    if (capi.ok) {
      await supabase
        .from("orders")
        .update({ meta_initiate_checkout_sent: true })
        .eq("id", order.id)
        .eq("meta_initiate_checkout_sent", false);
    }
    return NextResponse.json({ sent: capi.ok });
  } catch (e) {
    const err = e instanceof Error ? e : new Error(String(e));
    console.error("[POST /api/meta/initiate-checkout]", err);
